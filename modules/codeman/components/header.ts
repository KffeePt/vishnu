...
import { state } from '@vishnu/platform';
import { AppConfig } from '../../../codeman/config/app-config';
import { getRainbowColor, Colors } from '../../../codeman/singletons/shiva/core/utils';
  ...
  "88,d88  "88 88"   "88 888  "YeeP"   d888b Y8b Y8b "88 888 888 888                                                                                               
    ...
    if (mode !== 'auth' && mode !== 'welcome') {
        ...
        if (state.project.rootPath) {
            const projectName = path.basename(state.project.rootPath);
            contextLine += `${chalk.dim('Project:')} ${chalk.bold.white(projectName)}   `;
        }
