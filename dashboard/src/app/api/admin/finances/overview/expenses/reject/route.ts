...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find and remove expense from preview
    const expenseIndex = content.branches.preview.expenses.findIndex((e: any) => e.id === expenseId);
    if (expenseIndex === -1) {
      return NextResponse.json({ error: 'Expense not found in preview' }, { status: 404 });
    }

    const rejectedExpense = content.branches.preview.expenses[expenseIndex];
    content.branches.preview.expenses.splice(expenseIndex, 1);
