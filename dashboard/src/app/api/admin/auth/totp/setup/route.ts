...
import { getTotpStatusCached, invalidateTotpCache } from '@/lib/totpCache';
...
/** GET /api/admin/auth/totp/setup — check if TOTP is enabled for the user */
export async function GET(request: NextRequest) {
    try {
      ...
      issuer: 'Vishnu Control Center',
                ...
                // If decryption fails, just return enabled: false so user can restart
                return NextResponse.json({ enabled: false });
            }
        }
...
/** POST /api/admin/auth/totp/setup — generate a new TOTP secret and return QR code URI */
...
/** DELETE /api/admin/auth/totp/setup — remove TOTP for the user */
