...
import { getMissingPasskeyMessage, getWrongPanelPasskeyMessage, isCandymanPanelPasskey } from '@/lib/passkey-panels';
...
/** GET /api/staff/auth/webauthn/authenticate?userId=... — returns authentication options */
async function getHandler(request: NextRequest) {
    try {
        ...
        const passkeyDocs = (await getPasskeysForUserCached(userId)).filter((doc) => isCandymanPanelPasskey(doc.data()));

        if (passkeyDocs.length === 0) {
            return NextResponse.json({ error: getMissingPasskeyMessage('candyman') }, { status: 404 });
        }
        ...
        if (userPasskeys.length === 0) {
            return NextResponse.json({ error: 'Las llaves de acceso en este dispositivo están corruptas o las llaves maestras cambiaron. Por favor, elimina la llave en "Opciones" y vuelva a registrarla.' }, { status: 400 });
        }
...
/** POST /api/staff/auth/webauthn/authenticate — verifies assertion and creates session */
async function postHandler(request: NextRequest) {
    try {
        ...
        if (!isCandymanPanelPasskey(passkeyData)) {
            return NextResponse.json({ error: getWrongPanelPasskeyMessage('candyman') }, { status: 403 });
        }
        ...
        // Verify user permissions - ALLOW STAFF
        const auth = require('firebase-admin').auth();
        const userRecord = await auth.getUser(userId);
        const customClaims = userRecord.customClaims || {};

        const { isBootstrapOwner } = require('@/lib/ownerBootstrap');
        const tokenMock = { uid: userId, owner: customClaims.owner, admin: customClaims.admin, staff: customClaims.staff };
        if (customClaims.staff !== true && customClaims.admin !== true && customClaims.owner !== true && !(await isBootstrapOwner(tokenMock))) {
            return NextResponse.json({ error: 'User does not have required permissions: Staff access required' }, { status: 403 });
        }

        // Option 3: Device-Bound Key Unwrapping for Staff Passkey
        ...
        if (passkeyData.wrappedMasterPassword && secret) {
            try {
                ...
                const sessionPayload = {
                    ...
                    userId,
                    role: customClaims.staff ? 'staff' : (customClaims.admin ? 'admin' : 'owner'),
                    type: 'master-password-session'
                };

                encryptedSession = encryptData(sessionPayload, unwrappedMp);
            } catch (unwrapErr) {
                console.error('Failed to unwrap MP during staff passkey login (fallback needed):', unwrapErr);
            }
        }
