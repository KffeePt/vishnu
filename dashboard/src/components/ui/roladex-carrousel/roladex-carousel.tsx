...
interface RoladexCarouselProps {
  items: CarouselItem[];
  additionalImages?: Record<string, ProjectImage[]>; // Map of item id to additional images
}
...
export const RoladexCarousel: React.FC<RoladexCarouselProps> = ({ items, additionalImages = {} }) => {
  ...
  const handleCardClick = (index: number) => {
    ...
    if (offset === 0) {
      ...
      setFullscreenProject({
        ...
        mainImage: { url: item.imageUrl, title: item.photoTitle || item.title, subtitle: item.photoSubtitle || item.subtitle },
  ...
  return (
    <>
      <div className="w-full h-full flex items-center justify-center gap-2 sm:gap-8 font-sans">
        <div
          ref={scrollContainerRef}
          className="relative w-full max-w-[500px] h-[400px] cursor-grab active:cursor-grabbing"
            ...
            {items.map((item, index) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...getCardStyle(index),
                    transform: `${getCardStyle(index).transform} scale(0.7)`,
