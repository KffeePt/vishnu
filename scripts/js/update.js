...
if (!isManagedRepo(projectRoot, installInfo)) {
    log('Skipping release-channel update because this repo is not the managed stable install.');
    process.exit(0);
}

try {
    const currentMetadata = readVersionMetadata(projectRoot) ?? {};
    const currentVersion = currentMetadata.version ?? 'unknown';
    const currentBranch = getCurrentBranch(projectRoot) || '(detached)';
    ...
    const { tag: targetTag, metadata: targetMetadata } = resolveStableRelease(projectRoot);
    ...
    syncRepoToStableTag(projectRoot, targetTag);
    installRootDependencies(projectRoot);
