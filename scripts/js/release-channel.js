...
export function syncRepoToStableTag(repoRoot, tag, options = {}) {
    ...
    if (!isWorktreeClean(repoRoot)) {
        throw new Error('Stable updates require a clean worktree. Commit or stash your local changes before updating.');
    }
...
export function bunCommand() {
    const configuredPath = process.env.BUN_EXE;
...
export function installRootDependencies(repoRoot, options = {}) {
    const { stdio = 'inherit' } = options;
    execFileSync(bunCommand(), ['install', '--frozen-lockfile'], {
        cwd: repoRoot,
        stdio
    });
}
